...
import { Box, Center, Flex, Text, VStack, ScaleFade } from '@chakra-ui/react';
...
export const Project = (props: ProjectPageProps) => {
	...
	// this is the extremely centered content before the slides
				...
				{({ inView, ref }) => (
					<Center w="100%" h="629px" p={['5%', '5%', '10%', '20%']} ref={ref}>
						<ScaleFade in={inView} transition={{ enter: { duration: 0.7 } }}>
							<Text fontSize={['16px', '20px', '22px', '24px']}>{preSlideContent}</Text>
						</ScaleFade>
					</Center>
				)}
			</InView>
	...
	// these are the slides of images
	...
	// this is the extremely centered content after slides
		...
		<Box w="100%">
			<VStack alignItems="flex-start">
				<FullScreenStack gap={10}>
					<ScaleFade in transition={{ enter: { duration: 0.7 } }}>
						<VStack alignItems="flex-start" gap={2} maxW="982px">
							...
							<Text fontSize={['16px', '20px', '22px', '24px']} fontWeight="400" color="text.lightGrey">
								{projectDescription}
							</Text>
						</VStack>
					</ScaleFade>
